...
func main() {
	...
	*/
	...
	// var IntermittentWave uint32 = uint32(pwm.Top() >> 7) // 断続波の設定
	// var IntermittentWave uint32 = uint32(pwm.Top() >> 6) // 断続波の設定
	//	var IntermittentWave uint32 = uint32(pwm.Top() >> 5) // 断続波の設定
	//	var IntermittentWave uint32 = uint32(pwm.Top() >> 4) // 断続波の設定
	var IntermittentWave uint32 = uint32(pwm.Top() >> 3) // 断続波の設定 一番出力が大きかった。
	//	var IntermittentWave uint32 = uint32(pwm.Top() >> 2) // 断続波の設定

	var CarrierWave uint32 = uint32(pwm.Top() - IntermittentWave) // 搬送波の設定
